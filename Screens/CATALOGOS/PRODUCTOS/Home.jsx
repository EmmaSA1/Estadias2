...
export default function Home() {
    ...
    const handleAction = (accion, COD_PRODUCTO) => {
        setSelected(COD_PRODUCTO);
        setAccion(accion);
        console.log(accion);
        ...
        switch (accion) {
            ...
            case 11:
                ...
                QuitarBaja(COD_PRODUCTO);
                break;
            case 12:
                console.log("Dar de baja producto");
                PonerBaja(COD_PRODUCTO);
                break;
            case 14:
                console.log("Editar producto");
                openEditModal(COD_PRODUCTO);
                break;
            default:
    ...
    useEffect(() => {
        fetch(`${url}/Productos/regresarProducto.php?estado=${statusFilter === 'null' ? 'VIGENTES' : 'NO VIGENTES'}`)
            .then(response => {
                if (!response.ok) {
    ...
    const openEditModal = (COD_PRODUCTO) => {
        const datos = data.find(item => item.COD_PRODUCTO === COD_PRODUCTO);
    ...
    const handleEdit = async (values) => {
        ...
        const index = data.findIndex(item => item.COD_PRODUCTO === selected.COD_PRODUCTO);
        ...
        try {
            const response = await fetch(`${url}/Productos/editarProducto.php`, {
                method: 'POST',
                headers: {
                ...
                },
                {
                    ...
                    onPress: () => {
                        fetch(`${url}/Negocios/bajaNegocio.php`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    navigation.replace('SplashCatalogoProd', { accion: Comun.accion.Baja });
                                } else {
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: "Sí",
                    onPress: () => {
                        fetch(`${url}/Productos/altaProducto.php`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                            ...
                            .then(data => {
                                if (data.success) {
                                    navigation.replace('SplashCatalogoProd', { accion: Comun.accion.Alta });
                                } else {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.container3}>
                <Text style={styles.headerTitulo}>{Comun.nombreCatalogo[104]}</Text>
                            ...
                            .map((item, index) => (
                                <View key={index} style={styles.Contenido}>
                                    ...
                                    <View style={styles.iconContainer}>
                                        ...
                                        {statusFilter === 'baja' ? (
                                            <TouchableOpacity onPress={() => handleAction(Comun.accion.Alta, item.COD_PRODUCTO)}>
                                                <Icon name="checkmark-outline" size={25} color="black" />
                                            </TouchableOpacity>
                                        ) : (
                                            <TouchableOpacity onPress={() => handleAction(Comun.accion.Baja, item.COD_PRODUCTO)} disabled={statusFilter === 'baja'}>
            ...
            >
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        <Text style={styles.titleModal}>Modificar {Comun.nombreCatalogo[104]}</Text>
                        <ScrollView>
                            <FormProductos onSubmit={handleSubmit} action={accion} initialValues={{ ...selectedValues }} />
                        </ScrollView>
                    </View>
                </View>
            </Modal>
